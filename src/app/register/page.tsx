  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="hidden md:flex flex-col justify-center items-center w-1/2 bg-gradient-to-br from-[#8B4513] to-[#5B2415] text-white p-12 relative">
        <div className="absolute top-8 left-8 text-3xl font-extrabold tracking-wide opacity-30 select-none">JoaliStay</div>
        <div className="z-10">
          <h1 className="text-5xl font-extrabold mb-4 drop-shadow-lg">JoaliStay</h1>
          <p className="text-xl font-medium mb-8 opacity-90">Welcome to luxury redefined.<br/>Create your account below.</p>
          {/* Optionally add an illustration or logo here */}
        </div>
        <div className="absolute bottom-8 left-8 text-xs opacity-50">&copy; {new Date().getFullYear()} Joali Maldives</div>
      </div>
      {/* Right Panel: Form */}
      <div className="flex flex-1 justify-center items-center bg-gray-50 py-12 px-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 space-y-6">
          <h2 className="text-3xl font-bold text-center text-[#8B4513] mb-6">Create Account</h2>
          ...
          <div>
            <label className="block text-sm font-medium text-[#8B4513]">Confirm Password</label>
            <input
              type="password"
        ...
        <div className="flex flex-col items-center mt-6">
          ...
          <Link href="/">
            <span className="text-sm text-center text-gray-900 pt-2 cursor-pointer hover:underline">Back To Home Page</span>
          </Link>
        </div>
      </div>
    </div>
