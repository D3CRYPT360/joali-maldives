...
export default function ServicesPage() {
  ...
  async function fetchAll(staffOrgId?: number) {
    ...
    try {
      ...
      setOrgOptions(Array.isArray(orgsRes) ? orgsRes.map((o: any) => ({ id: o.id, name: o.name })) : []);
      setServiceTypeOptions(Array.isArray(typesRes) ? typesRes.map((t: any) => ({ id: t.id, name: t.name })) : []);
    } catch (err) {
  ...
  filteredServices = filteredServices.filter((svc) => {
    ...
    return (
      ...
      (svc.description && svc.description.toLowerCase().includes(searchLower)) ||
      (svc.organization && svc.organization.name.toLowerCase().includes(searchLower)) ||
      (svc.serviceType && svc.serviceType.name.toLowerCase().includes(searchLower))
    );
  });
                    ...
                    {filteredServices.map((svc) => (
                      <tr key={svc.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {svc.imageUrl ? (
                            <img src={svc.imageUrl} alt={svc.name} className="h-12 w-20 object-cover rounded" />
                          ) : (
                            "-"
                          )}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">{svc.capacity ?? "-"}</td>
                        <td className="px-6 py-4 whitespace-nowrap">{svc.durationInMinutes ?? "-"}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
